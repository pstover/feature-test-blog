...
public class ProdServerForIDE {
	...
	public ProdServerForIDE(boolean usePortZero) {
		...
		srcPaths.add(new VirtualFileImpl(directory+"/featuretestapp/src/main/java"));
		srcPaths.add(new VirtualFileImpl(directory+"/featuretestapp-dev/src/main/java"));
		
//		VirtualFile metaFile = new VirtualFileImpl(directory + "/featuretestapp/src/main/resources/appmetadev.txt");
//		log.info("LOADING from meta file="+metaFile.getCanonicalPath());
	...
	public static String modifyForIDE(String filePath1) {
        ...
        if(filePath1.contains("featuretestapp-dev")) {
            //eclipse starts in featuretestapp-dev so move one directory back
